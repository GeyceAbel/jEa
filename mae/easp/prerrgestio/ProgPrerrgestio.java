		...
		public class LinkAcconsulta extends Action
		{
			...
			public void onAction()
			{
				...
				try {
					...
					out.close();
					String err2send = mae.general.Base64.encodeBytes(bytes);^
					*/
					String email = "";
					if (!vvemail.isNull()) email = vvemail.getString();
					...
					for (Enumeration e = eparam.keys(); e.hasMoreElements(); ) {
						...
						if (o != null) lines.add (k.toString()+": "+o.toString());
					}
					...
					if (bOk) result = az.getContenido();
					if (!bOk || result.startsWith("-1"))  {
						if (!bOk) Maefc.message("Error enviando consulta técnica.\nPóngase en contacto con su servicio técnico.\n"+az.getError(),"Error envío",Maefc.ERROR_MESSAGE);
						else Maefc.message("Error enviando consulta técnica.\nPóngase en contacto con su servicio técnico.\n"+result.substring(3),"Error envío",Maefc.ERROR_MESSAGE);
					} else {
