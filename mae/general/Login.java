...
public class Login implements LoginListener{
  ...
  Login(Aplication apl){
    SystemView.setGui(true);
    //Leemos .properties
    properties(apl);

    ld=new LoginDialog(this,apl);
...
//    MAE.setConsulado(1);
  ...
  public boolean accept(){
    try {
      ...
      ResultSet rs=dbc.executeQuery(st,"select USPASSWD from USUARIO where "+
                                    funcio+"(USLOGIN)='"+mylogin+"'", false);
      if (rs.next()) {
        String pass=rs.getString("USPASSWD");
        rs.close();

        if ((pass!=null && pass.toUpperCase().equals(ld.getPassword().toUpperCase())) ||
            (pass==null && ld.getPassword().equals(""))) {
